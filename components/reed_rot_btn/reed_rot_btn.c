 ...
 * @brief   ESP32D library to interface with a dual pole reed contact, a
 * mechanical rotary encoder and buttons
 *
...
extern const uint8_t ulp_reed_rot_btn_bin_start[] asm(
    "_binary_ulp_reed_rot_btn_bin_start");
extern const uint8_t ulp_reed_rot_btn_bin_end[] asm(
    "_binary_ulp_reed_rot_btn_bin_end");
...
static const uint8_t rtc2gpio_map[16] = {36, 37, 38, 39, 34, 35, 25, 26,
                                         33, 32, 4,  0,  2,  15, 13};
 ...
 * @note IOs must have an external pull-down resistor (2.2 Meg Ohms) and switchs
 * to Vcc
 *
 */
...
#if IDF_VER_MAJOR < 5
    ESP_ERROR_CHECK(
        rtc_isr_register(reed_rot_btn_isr, NULL, RTC_CNTL_SAR_INT_ST_M));
    // ESP_ERROR_CHECK(rtc_isr_register(reed_rot_btn_isr, NULL,
...
esp_err_t reed_rot_btn_read(unsigned int *p_key_code, unsigned int *reed_delay,
                            int timeout) {
    recv_task = xTaskGetCurrentTaskHandle();
 ...
 */
{
    ...
    time_curr =
        xTaskGetTickCountFromISR();  // assign time as quickly as possible
    ...
    reed_rot_btn_code =
        0xffff &
        ulp_evnt;  // ulp variables are 32 bit, but only the lower 16 are valid
    ...
    xTaskNotifyFromISR(recv_task, reed_rot_btn_code, eSetValueWithoutOverwrite,
...
static void init_ulp_prog(void) {
    esp_err_t err = ulp_load_binary(
        0, ulp_reed_rot_btn_bin_start,
        (ulp_reed_rot_btn_bin_end - ulp_reed_rot_btn_bin_start) /
            sizeof(uint32_t));
    ESP_ERROR_CHECK(err);
...
static void init_rtcios(void) {
    ...
    rtc_gpio_set_direction(rtc2gpio_map[CONFIG_REED_DEBOUNCE_RTC_GPIO_NUM],
...
#endif
    ESP_LOGD(TAG, "RTCIO Mask 0x%x",
             (IO_REED0_MASK | IO_REED1_MASK | AB_RTC_GPIO_MASK |
              CONFIG_BTN_RTC_GPIO_MASK));
}
...
static void init_rtcio(unsigned int rtc_io) {
    ...
    rtc_gpio_set_direction(
        gpio, RTC_GPIO_MODE_INPUT_ONLY);  // direction input (also needed for
                                          // input only pins)
    rtc_gpio_hold_dis(gpio);
    rtc_gpio_pulldown_dis(
        gpio);  // Some pins do not feature poll-up, pull-down, so never use
